...
from maraboupy import MarabouNetworkNNet  #pytype: disable=import-error
...
class Network:
    ...
    end_relu    -   Does the last layer have ReLU or not
    num_layers  -   Holds the total number of layers in the network, including
                    input and output
    layer_sizes -   A list with sizes of each layer, including output and input
                    layer
    ...
    def eval( self, inp ):
        ...
        1.  The vector of return values, or stack if input is stack.
        2.  A list with the values at each layer. At each layer, if the layer
            ...
            the output of the linear layer. Again, if input is a stack, so are
            these.
        """
        vals = [ inp ]
        cval = inp
    ...
    def dump_npz( self, fname, extra_entries = {} ):
        ...
        if config.DEBUG:
            l.info("Layer sizes of network being dumped: ",
                save_dict['layer_sizes']
            )
            l.info("Total size: ", sum( save_dict['layer_sizes'] ))
            l.info("Dumping to: ", fname)
...
if __name__ == "__main__":
    ...
    l.info("Test number: {}".format(test_no))

    if test_no == 0:
        """
        Load all nnet, onnx files, run random inputs, make sure they are same
        """
        l.info("Comparing nnet and onnx loaded networks")

        N = 10000
        rng = np.random.default_rng( seed = config.RNG_SEED )
        inps = rng.uniform(-1, 1, (N, 5))
        
        for i in range(1, 6):
            for j in range(1, 10):
                l.info("Network {},{} of 5,9".format(i,j))
                
                fname = "../networks/ACASXU_run2a_{}_{}_batch_2000".format(i,j)
                net_nnet = load_nnet( "{}.nnet".format( fname )) 
                net_onnx = load_onnx( "{}.onnx".format( fname )) 

                nnet_out, _ = net_nnet.eval(inps)
                onnx_out, _ = net_onnx.eval(inps)
                
                if not np.allclose( nnet_out, onnx_out ):
                    l.error("Discrepancy in output values")

                    bad_inp_idx = np.nonzero(
                        np.logical_not( np.allclose( nnet_out, onnx_out )) 
                    )[0][0]
                    l.error("Inputs: {}".format( inps[bad_inp_idx,:] ))
                    l.error("Outputs: {}, {}".format( 
                        nnet_out[bad_inp_idx,:], onnx_out[bad_inp_idx,:] 
                    ))
    ...
    if test_no == 1:
        """
        Load all nnet files and run on random inputs
        """
        l.info("Comparing nnet and onnx loaded networks")
    ...
    if test_no == 2:
        ...
        net = load_onnx( '/tmp/mnist_relu_3_50.onnx' )
        l.info("Net is: {}".format( str( net )))
                
    else:
        l.error("Unknown test number {}".format( test_no ))
